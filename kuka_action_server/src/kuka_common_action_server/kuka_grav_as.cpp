  ...
  /*  if (action_name == (goal->action_name))
    {
        ...
        while(ros::ok()) {
            ...
            //ROS_INFO_STREAM_THROTTLE(1,"des_j_sti[0]: " << des_j_stiffness[0]);
            // ROS_INFO_STREAM_THROTTLE(1,"j_stiffness[0]: " << j_stiffness[0]);
            ///--- Stop sending when the desired stiffness is reached ---///
            Eigen::VectorXd stiff_diff = j_stiffness - des_j_stiffness;
            double stiff_err =  abs(stiff_diff.sum());

            ROS_INFO_STREAM_THROTTLE(1,"Current error to desired stiffness: " << stiff_err);
            if(stiff_err < 0.5){
                ROS_INFO_STREAM_THROTTLE(1,'Desired Stiffness REACHED, you can manipulate the robot now...');
                break;
            }
